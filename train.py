...
def get_args():
    ...
    parser.add_argument(
        "--seed", type=int, default=443, metavar="S", help="random seed (default: 443)"
    )
    parser.add_argument(
...
def validation(args, model, device, val_loader, optimizer, epoch, criterion, cur_psnr):
    ...
    print(
        "Validation set: Average PSNR: {:.4f}, mu_law: {:.4f}".format(
    ...
    if val_mu_psnr.avg > cur_psnr[0]:
        ...
        with open(os.path.join(args.logdir, "best_checkpoint.json"), "w") as f:
            ...
            f.write(
                "Validation set: Average PSNR: {:.4f}, PSNR_mu_law: {:.4f}\n".format(
                    val_psnr.avg, val_mu_psnr.avg
                )
            )
...
def test_single_img(model, img_dataset, device):
    dataloader = DataLoader(
        dataset=img_dataset, batch_size=1, num_workers=1, shuffle=False
    )
    # model.eval()
    with torch.no_grad():
        for batch_data in dataloader:
            ...
            img_dataset.update_result(
                torch.squeeze(output.detach().cpu()).numpy().astype(np.float32)
            )
...
def test(args, model, device, optimizer, epoch, cur_psnr, **kwargs):
    ...
    for idx, img_dataset in enumerate(test_datasets):
        ...
        pred_img = np.clip(pred_img * 255.0, 0.0, 255.0).transpose(1, 2, 0)
        label = np.clip(label * 255.0, 0.0, 255.0).transpose(1, 2, 0)
        pred_img_mu = np.clip(pred_img_mu * 255.0, 0.0, 255.0).transpose(1, 2, 0)
        label_mu = np.clip(label_mu * 255.0, 0.0, 255.0).transpose(1, 2, 0)
    ...
    print(
        "==Validation==\tPSNR_l: {:.4f}\t PSNR_mu: {:.4f}\t SSIM_l: {:.4f}\t SSIM_mu: {:.4f}".format(
            psnr_l.avg, psnr_mu.avg, ssim_l.avg, ssim_mu.avg
        )
    )
    ...
    if psnr_mu.avg > cur_psnr[0]:
        ...
        with open(os.path.join(args.logdir, "best_checkpoint.json"), "w") as f:
            f.write("best epoch:" + str(epoch) + "\n")
            f.write(
                "Validation set: Average PSNR: {:.4f}, PSNR_mu: {:.4f}, SSIM_l: {:.4f}, SSIM_mu: {:.4f}\n".format(
                    psnr_l.avg, psnr_mu.avg, ssim_l.avg, ssim_mu.avg
                )
            )
